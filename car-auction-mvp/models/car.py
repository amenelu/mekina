...
car_equipment_association = db.Table(
    "car_equipment",
...
class Car(db.Model):
    ...
    listing_type = db.Column(
        db.String(50), default="auction", nullable=False
    )  # 'auction', 'sale', 'rental'
    ...
    auction = db.relationship(
        "Auction", backref="car", uselist=False, cascade="all, delete-orphan"
    )
    rental_listing = db.relationship(
        "RentalListing", backref="car", uselist=False, cascade="all, delete-orphan"
    )
    ...
    equipment = db.relationship(
        "Equipment",
        secondary=car_equipment_association,
        lazy="subquery",
    ...
    def primary_image_url(self):
        ...
        if (
            self.images
            and self.images[0].image_url
            and "/static/" in self.images[0].image_url
        ):
